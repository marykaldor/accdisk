...
C--   the wavelength scale supplied above.
     ...
     &     xi1,xi2,broad,q,angi,                       ! disk parameters
...
c         write(*,*) keyword,fluxunits
         m=m+1
      elseif (keyword.eq.'Q') then ! ----- circular disk
         read(value,*) q
         do i=1,maxl
            if (parlabel(i).eq.'Q') parlabel(i)='check'
         enddo
c         write(*,*) keyword,q
         m=m+1
...
C--   problem leads to a warning but the code continues. 
      ...
      if (nstep.gt.maxstep) then
         write(*,520) MAXSTEP,MAXSTEP 
 520     format(/,' WARNING: NSTEP cannot exceed ',i4,
     &        '. Reseting NSTEP = ',i4,'.')
      endif
...
C     Angles for spiral brightness patterm
      ...
      ELSE
         ...
         AOBS=-AOBS/57.29578
      ENDIF
      TANPITCH=TAN(PITCH/57.29578)
      SIG=(PI/180.)*WIDTH/SQRT(8.*LOG(2.))
...
C--   CPU intensive loops.
      ...
      PHISTEP = 2.*3.14159/NSTEP       ! phi step size
