...
# Uses coefficients of parabola fit, creates curve, finds maximum and half maximum without continuum, adds continuum in
# after to hit the proper scaling with respect to the continuum-added model.
...
# Find the intersection between spectrum and half max line, finds the params coordinates of those locations, then takes
# the difference of those to find the FWHM. LineString allows for interpolation between points so that we can find an
# exact intersection because sometimes there is no params value for an exact y = 0.5 value.
...
# Selects area around rough intersection, linearizes the model with the surrounding 20 points, finds intersection with
# FWHM line, takes difference of points to find FWHM.
    it1 = 0
    lowlin1 = []
    lowlin2 = []
